...
This file is identical to the Strands version. The rules engine is
framework-agnostic: it's pure Python logic with no dependency on
Strands or LangGraph.

The difference is HOW these rules are enforced:
  Strands:   HookProvider with BeforeToolCallEvent
  LangGraph: Guardrail node between LLM and tool execution
"""
...
def valid_dates_check(ctx: dict) -> bool:
    return (
        ctx.get("check_in")
        and ctx.get("check_out")
        and ctx["check_in"] < ctx["check_out"]
    )
